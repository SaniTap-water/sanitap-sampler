...
*Annex D to the SaniTap Water Quality Testing Protocol v2.2 (section 6.4). Applies to monitoring under the Gold Standard "Safe Drinking Water Supply" (SDWS) methodology v2.0.*
...
| 2 | Household | The **field rule** (section 6.4): the sampler counts the households served by the source (K); the tool draws N random numbers between 1 and K (default 5, plus 2 replacements) from the round seed, the source id and K, and for each number k the k-th household met when walking from the source is sampled. Sterile sampling equipment; no flaming of spouts; the purge and every sampling step are recorded on the mWater form (SDWS 22 PoU) with the record id. |

Replacement sources and households are used strictly in the listed order and only when a primary unit cannot be sampled (pump broken, household absent after two visits, refusal). The reason is recorded in mWater. The tool flags a selected source that lies more than 25 km from every other selected source and from the district town; such a source is replaced in the field only if it proves unreachable, never automatically, and the reason is recorded.
...
For every draw the tool produces a **sampling record (PDF)** that narrates the method, frame, randomness, design check and selection for a validator, prints the seed and the frame hash in its footer, and carries the complete machine-readable audit (record id, timestamp, who drew it, seed, algorithm and tool version, source and SHA-256 hash of the frame, eligibility counts, all parameters, stage-1 numbers, the selected sources with their weights and hit positions, the replacements, the reach check and the field rule with the numbers generated once K was entered) as an attached file whose SHA-256 is printed on every page. The tool also produces a **selection file (Excel)** with the same sources, replacements and parameters as data for the team. Both, with a selection CSV, are filed under `records/<round>/<stratum>/` in the tool's repository, published at `https://sanitap-water.github.io/sanitap-sampler/records/` and copied to the monitoring report folder of the round; they carry no coordinates (the frame file and the field sheets stay in the private archive). The link from mWater to the record is the **record id** (`<round>-<stratum>-<seed>`) entered on each PoU form response. **The sampling record is the evidence of random selection retained for the validation and verification body (VVB).**
