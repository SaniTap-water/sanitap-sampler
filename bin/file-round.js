...
const files = [[pdfPath, 'sampling-record.pdf'], [jsonPath, 'audit.json'], [csvPath, 'selection.csv']].concat(framePath ? [[framePath, 'frame.csv']] : []);
console.log('Record ' + audit.record_id + ' -> records/' + round + '/' + stratum + '/'); console.log('  audit JSON SHA-256 ' + auditSha + ' (matches PDF)'); console.log('  frame SHA-256 ' + frameSha + (framePath ? ' (frame.csv verified)' : ' (frame file not supplied)'));
...
fs.appendFileSync(idx, `| ${round} | ${stratum} | ${audit.timestamp} | \`${audit.seed}\` | \`${auditSha}\` | \`${frameSha || ''}\` | [PDF](${round}/${stratum}/sampling-record.pdf) · [audit](${round}/${stratum}/audit.json) · [CSV](${round}/${stratum}/selection.csv)${framePath ? ' · [frame](' + round + '/' + stratum + '/frame.csv)' : ''} |\n`);
